...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Awake()
    {
        playerInputs=GameObject.FindGameObjectWithTag("Game Controls").GetComponent<PlayerInput>();
    ...
    private void initControls(){
        crouchButton=playerInputs.actions["Crouch"];
        moveControls=playerInputs.actions["Move"];
        sprintButton=playerInputs.actions["Sprint"];
    }
    ...
    private void Update()
    {
        ...
        if (enableCrouch)
        {
            if(crouchButton.WasPressedThisFrame() && !holdToCrouch)
            {
                Crouch();
            }
            ...
            else if(crouchButton.WasReleasedThisFrame() && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
            }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector2 rawVelocity=moveControls.ReadValue<Vector2>();
            Vector3 targetVelocity=new Vector3(rawVelocity.x, 0,rawVelocity.y);
            ...
            if (enableSprint && sprintButton.IsPressed() && sprintRemaining > 0f && !isSprintCooldown)
            {
